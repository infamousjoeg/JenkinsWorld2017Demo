...
function hostfactory_create(){
    ...
    local time_seconds=$(date -d "+ 20 seconds" +%S)
	  local hostfactory=$(curl -s -k -X POST -H "Authorization: Token token=\"$token\"" "https://conjur-master/api/host_factories/jenkins%2Fmasters/tokens?expiration=${date}T${time_hours}%3A${time_minutes}%3A${time_seconds}-05%3A00")

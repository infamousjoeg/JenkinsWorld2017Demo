...
function hostfactory_create(){
    ...
    local auth_token=$(curl -s -k -H "Content-Type: text/plain" -X POST -d "$api" https://docker/api/authn/users/admin/authenticate)
    local token=$(echo -n $auth_token | base64 | tr -d '\r\n')
    printf "\nThe token is: $token\n"
    local hostfactory_search=$(curl -s -k -X GET -H "Authorization: Token token=\"$token\"" https://docker/api/authz/admin/resources/host_factory | jq '.[] | .id ')
